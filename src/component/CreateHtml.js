...
export const StartComponent = ({id}) => {
...
const selectValue = () => {
  ...
  const controlerClick = (controler) => {
    gameStartScript(controler,select,gameBoard,matrix, h2, status)
  }
  ...
  return (
    ...
    <div className='divStyle'>
        <div><button onClick={() => {
          ...
          gameBoardCreateScript(select,gameBoard,matrix,gameSettings,status) 
        }}>Start</button></div>
        <select  ref={select}>
